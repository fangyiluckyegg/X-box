...
| `docs/deployment/prod-mac-runbook.md` | **改** | §6 Ollama 段：由「容器内 CPU 提醒」改为「宿主原生 Ollama + 启动顺序 + host.docker.internal」 |
| `docs/deployment/Mac-mini-deployment-guide.md` | **改（轻微）** | §6 已正确，补充「编排层已改为 host.docker.internal，无需手动改 env」 |
| `docs/architecture/architecture_current_topology.mmd` | **改** | `LL` 节点改为「宿主原生 Ollama（host.docker.internal）」 |
| `docs/architecture/architecture_target_topology.mmd` | **改** | `LLp` 节点说明同步为宿主原生（可选） |
...
- `docs/deployment/prod-mac-runbook.md`：§6「Ollama 提醒（容器内 CPU）」整体改写为「宿主原生 Ollama」：引用 `host.docker.internal:11434`、启动顺序（宿主 Ollama 先起）、`OLLAMA_HOST=0.0.0.0:11434` 必要性、防火墙提示。
...
- `docs/architecture/ollama-host-migration-design.md`：本文件（主交付）。
