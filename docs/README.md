...
网关 80/443 实际绑定 `0.0.0.0`（同一局域网设备可经宿主内网 IP 访问）；MySQL/Redis 仍仅绑回环 `127.0.0.1`。公网访问不经由网关 80/443，而是经 frp 内网穿透将 PHP 容器 `127.0.0.1:1181` 暴露到自有 VPS（TLS 在 VPS frps 终止）。统一局域网入口是 nginx 网关（80/443）。
