...
> 配套产物：`docker-compose.prod.yml`（已做 Apple Silicon 适配）、`.env.prod`（密钥模板）、`gateway/nginx/conf.d/prj.conf`（已加固）、`docs/code-review-report.md`（代码审查）、`docs/qa-verification.md`（测试与验证手册）。
...
- `docs/qa-verification.md`（QA 验证与测试手册，由 QA 工程师产出）
