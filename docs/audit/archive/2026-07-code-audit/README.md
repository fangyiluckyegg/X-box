...
> 归档时间：2026-07-12
> 当前路径：`docs/audit/archive/2026-07-code-audit/`（2026-07-20 文档整理时由旧 `archive/` 目录整体并入 `audit/archive/`）
> 归档原因：项目进入微优化阶段，将原代码审计产出从交付物暂存区迁移至归档目录，作为历史上下文长期保留，不再参与日常交付物清单。
> 并入说明（2026-07-20）：xbox 系列历史清理 / 对齐交付报告已随本次文档整理并入 `docs/audit/archive/2026-07-20/`；本代码审计目录现统一位于 `docs/audit/archive/` 体系下。
...
- 部署文档 `docs/deployment/prod-mac-runbook.md` 与此处的整改背景互补。
- 基础设施脚本 `db/mysql_init/migrate_role.sql`、`db/mysql_scripts/docker-entrypoint-wrapper.sh` 的"为何存在"可在此找到原始问题定义。
- 2026-07-20 的 xbox 清理 / 对齐交付物见 `../2026-07-20/`；更早的 2026-07-14 一轮审计日志见 `../2026-07-14/`。
