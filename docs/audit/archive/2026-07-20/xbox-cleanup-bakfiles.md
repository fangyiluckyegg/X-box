...
## 保留文件（主动确认未误删）
- `.env.dev.example`（被 `docs/deployment/prod-mac-runbook.md` 作为 `cp .env.dev.example .env.dev` 模板引用）
- `.env.prod.example`（被 `docs/deployment/prod-mac-runbook.md` 作为 `cp .env.prod.example .env.prod` 模板引用）
...
| 行 | 修改前 | 修改后 |
|---|---|---|
| .env.dev.example（L84） | `# 是否可删除？` | `# 保留：被 docs/deployment/prod-mac-runbook.md 作为 cp 模板引用` |
| .env.prod.bak（L85） | `# 是否可删除？` | 整行删除 |
| .env.prod.example（L86） | `# 是否可删除？` | `# 保留：被 docs/deployment/prod-mac-runbook.md 作为 cp 模板引用` |
...
- 删除：10 个 bak 文件（见上表）
- 修改：`项目开发说明`
- 报告：`docs/audit/archive/2026-07-20/xbox-cleanup-bakfiles.md`
